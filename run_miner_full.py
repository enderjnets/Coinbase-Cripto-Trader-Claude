#!/usr/bin/env python3
"""
Strategy Miner - Ejecución Completa con 22 CPUs
Configuración óptima para encontrar estrategias ganadoras
"""
...
print("\n" + "="*80)
print("🧬 STRATEGY MINER - EJECUCIÓN COMPLETA CON 22 CPUs")
print("="*80 + "\n")
...
print(f"   • Generaciones: 50")
print(f"   • Risk Level: LOW")
print(f"   • CPUs: 22 (distribuidos)")
print(f"   • Tiempo estimado: ~11 minutos\n")
...
def show_progress(msg_type, data):
    global gen_start_time, generation_times
    ...
    if msg_type == "START_GEN":
        ...
        if generation_times:
            ...
            remaining_gens = 50 - gen
            eta = avg_time * remaining_gens
            eta_min = int(eta / 60)
            eta_sec = int(eta % 60)
            print(f"\n🧬 Gen {gen:2d}/50 | Transcurrido: {int(elapsed/60):2d}m {int(elapsed%60):2d}s | ETA: {eta_min:2d}m {eta_sec:2d}s", flush=True)
        else:
            print(f"\n🧬 Gen {gen:2d}/50 | Transcurrido: {int(elapsed/60):2d}m {int(elapsed%60):2d}s", flush=True)
        ...
        print(f"   {emoji} PnL: ${pnl:>8,.2f} | Trades: {trades:>3d} | Win: {win_rate:>5.1f}% | {gen_time:>4.1f}s", flush=True)
...
if best_pnl > 3000:
    ...
    print("⚠️  PnL negativo - considera ejecutar de nuevo con más generaciones")
...
with open('last_miner_result.json', 'w') as f:
    json.dump(result, f, indent=2)

print(f"   ✅ Guardado en: last_miner_result.json")
