                ...
                try:
                    ...
                    try:
                        ...
                        if os.path.exists(env_path):
                            ...
                            log_cb(f"✅ Variables cargadas desde .env")
                    except ImportError:
                        pass  # python-dotenv no instalado
                    except Exception as e:
                        log_cb(f"⚠️  No se pudo cargar .env: {e}")

                    ray_addr = os.environ.get("RAY_ADDRESS", "auto")
                    log_cb(f"🌍 Conectando a Ray en: {ray_addr}...")

                    # NO usar runtime_env - el código ya existe en ambas máquinas
                    # Esto evita el overhead de sincronizar miles de archivos
                    runtime_env = None
                    ...
                    if not init_done.wait(timeout=30):
                        log_cb("⚠️ Timeout conectando al cluster.")
