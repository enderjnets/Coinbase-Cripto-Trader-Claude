...
class TradingBot:
    ...
    async def execute_trade(self, signal, product_id, price):
        """
        Executes a trade (Paper or Live).
        """
        # Risk Calc
        ...
        if self.mode == "PAPER":
            # PAPER TRADING LOGIC
            if signal == "BUY":
                if self.paper_balance < size_usd:
                    self.log("❌ Insufficient Paper Funds.")
                    return
                ...
                self.active_positions[product_id] = {
                    ...
                    'current_price': price,
                    'stop_loss': sl,
                    'size': size_usd / price, # Unit size
                    'size_usd': size_usd,
                    'pnl': 0.0,
                    'entry_time': datetime.now()
                }
                self.log(f"✅ PAPER BUY Executed. New Balance: ${self.paper_balance:.2f}")

            elif signal == "SELL":
                 # Simple Mock: Close position if exists
                 if product_id in self.active_positions:
                     ...
                     self.log(f"✅ PAPER SELL (Close). PnL: ${pnl:.2f} ({pnl_percent:.2f}%). New Balance: ${self.paper_balance:.2f}")
