...
def run_optimizer_process(data_dict, param_ranges, risk_level, opt_method,
    ...
    try:
        ...
        # --- RAY INITIALIZATION ---
        # Initialize Ray globally for this process to ensure Miner/Genetic uses it
        log_cb("🔍 Inicializando Entorno Ray...")
        if ray.is_initialized():
            ...
            if not force_local:
                try:
                    ...
                    # Google Drive syncing causes hanging when Ray tries to zip the folder.
                    # Solved by copying only code to a local, non-synced temp dir.
                    import shutil
                    temp_context_dir = os.path.expanduser("~/.bittrader_head/ray_context")
                    
                    log_cb("📦 Preparando contexto ligero para Ray...")
                    ...
                    log_cb(f"📋 Contexto creado con {files_copied} archivos en {temp_context_dir}")
                    
                    ray_addr = os.environ.get("RAY_ADDRESS")
                    if ray_addr:
                        log_cb(f"🌍 Conectando a Cluster: {ray_addr}")
                    
                    # --- RESTORING RUNTIME ENV (LITE) ---
                    # We use the temp_context_dir as working_dir so Ray syncs only .py files.
                    runtime_env = {
                        "working_dir": temp_context_dir,
                    }
                    ...
                    log_cb(f"✅ Conectado a Cluster Ray ({cpus} CPUs) con Sincronización de Código")
                    connected = True
                except Exception as e:
                    log_cb(f"⚠️ Cluster no encontrado o error de sincronización: {e}")
