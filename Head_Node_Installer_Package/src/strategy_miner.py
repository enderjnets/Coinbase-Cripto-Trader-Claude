...
class StrategyMiner:
    ...
    def evaluate_population(self, population, progress_cb=None, cancel_event=None):
        ...
        if ray.is_initialized() and not self.force_local:
            ...
            if not hasattr(self, "http_server_started"):
                ...
                try:
                    ...
                    def get_reachable_ip():
                        ...
                        try:
                            # Fallback: Get all IPs and pick the one that looks like Tailscale or LAN
                            import socket
            ...
            # The payload is now a URL STRING
            ...
            debug_log(f"Starting evaluation with HTTP Load: {data_payload}")
            
            # Pass the URL explicitly
            futures = [run_backtest_task.remote(data_payload, self.risk_level, genome, "DYNAMIC") for genome in population]
                ...
                for ref in done:
                    ...
                    try:
                        ...
                        if r_count < 3: # Max 3 retries
                            ...
                            new_ref = run_backtest_task.remote(data_payload, self.risk_level, genome, "DYNAMIC")
        ...
        for i, res in enumerate(results_raw):
            if isinstance(res, dict) and 'metrics' in res:
                ...
                fitness_scores.append((population[i], pnl))
            else:
                fitness_scores.append((population[i], -9999))
    ...
    def run(self, progress_callback=None, cancel_event=None):
        ...
        for gen in range(self.generations):
            ...
            if progress_callback:
                progress_callback("BEST_GEN", {'gen': gen, 'pnl': current_best[1], 'genome': current_best[0]})
