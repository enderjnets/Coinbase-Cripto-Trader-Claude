...
class GridOptimizer:
    ...
    def optimize(self, df, param_ranges, risk_level="LOW", progress_callback=None, cancel_event=None, log_callback=None, strategy_code="HYBRID"):
        ...
        try:
            ...
            # Dispatch ALL tasks
            log(f"🚀 Despachando {len(grid)} tareas a los workers (Estrategia: {strategy_code})...")
            futures = [run_backtest_task.remote(df_ref, risk_level, params, strategy_code) for params in grid]
    ...
    def optimize(self, df, param_ranges, risk_level="LOW", progress_callback=None, cancel_event=None, log_callback=None, strategy_code="HYBRID"):
        ...
        try:
            ...
            for gen in range(start_gen, self.generations):
                ...
                # Evaluate population
                futures = [run_backtest_task.remote(df_ref, risk_level, params, strategy_code) for params in population]
                gen_results = []
