...
class StrategyMiner:
    ...
    def generate_random_genome(self):
        ...
        # Direction: LONG for spot, LONG/SHORT for futures
        if self.market_type == "FUTURES":
            direction = random.choice(["LONG", "SHORT"])
            leverage = random.choice([1, 2, 3, 5, 10]) if self.max_leverage > 1 else 1
    ...
    def mutate(self, genome, generation=0, diversity_score=0.5):
        ...
        elif roll < adaptive_rate * 0.80:
            # Mutate direction (futures only)
            if self.market_type == "FUTURES":
                mutated["params"]["direction"] = random.choice(["LONG", "SHORT"])
                if self.max_leverage > 1:
                    mutated["params"]["leverage"] = random.choice([1, 2, 3, 5, 10])
                    mutated["params"]["leverage"] = min(mutated["params"]["leverage"], self.max_leverage)
    ...
    def crossover(self, p1, p2):
        ...
        # Direction and leverage: inherit from random parent (50/50)
        donor = random.choice([p1, p2])
        child["params"]["direction"] = donor.get("params", {}).get("direction", "LONG")
        child["params"]["leverage"] = donor.get("params", {}).get("leverage", 1)
        ...
        if HAS_NUMBA:
            try:
                return numba_evaluate_population(
                    ...
                    market_type=self.market_type, max_leverage=self.max_leverage,
