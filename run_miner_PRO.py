#!/usr/bin/env python3
"""
Strategy Miner - MACBOOK PRO
Configuración: MEDIUM risk, más generaciones
"""
...
print("\n" + "="*80)
print("🧬 STRATEGY MINER - MACBOOK PRO (MEDIUM RISK)")
print("="*80 + "\n")
...
# Configuración MacBook Pro: MEDIUM risk, más generaciones
pop_size = 40
generations = 30

print("⚙️  Configuración MacBook PRO:")
print(f"   Población: {pop_size}")
print(f"   Generaciones: {generations}")
print(f"   Risk Level: MEDIUM")
print(f"   Objetivo: Estrategias moderadas con buen balance")
print(f"   Tiempo estimado: 40-50 minutos\n")
...
def show_progress(msg_type, data):
    ...
    if msg_type == "START_GEN":
        ...
        print(f"\n{'='*70}")
        print(f"🧬 Gen {gen}/{generations} | ⏱️ {int(elapsed/60)}m | ETA: ~{remaining_mins}m | 💻 MacBook PRO")
        print(f"{'='*70}")

        # Escribir archivo de status
        with open('STATUS_PRO.txt', 'w') as f:
...
print("="*80)
print("⚡ INICIANDO MINERÍA (MacBook PRO - MEDIUM RISK)")
print("="*80)
print("ℹ️  Buscando estrategias con balance riesgo/retorno moderado")
print("ℹ️  Este proceso tomará ~40-50 minutos\n")

best_genome, best_pnl = miner.run(progress_callback=show_progress)
total_time = time.time() - start_time

print("\n" + "="*80)
print("🏆 RESULTADOS FINALES - MACBOOK PRO")
print("="*80 + "\n")
...
if best_ever['pnl'] > 0:
    ...
    result_file = f"BEST_STRATEGY_PRO_{timestamp}.json"
    with open(result_file, 'w') as f:
        json.dump({
            'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
            'machine': socket.gethostname(),
            'mode': 'MACBOOK_PRO_MEDIUM',
            'pnl': best_ever['pnl'],
...
all_file = f"all_strategies_PRO_{int(time.time())}.json"
...
print("="*80)
print(f"💻 MacBook PRO - Búsqueda completada")
print("="*80 + "\n")
