#!/bin/bash
#
# 🎁 Bittrader Worker Installer v2.6 (Mobile Worker Edition)
# Optimized for mobile workers with automatic reconnection and resilience
# Includes automatic Python 3.9.6 installation, Google Drive sync, and code verification
#
...
DEFAULT_HEAD_IP=""  # Leave empty to prompt user for MacBook Pro Head IP
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║    🎁 BITTRADER WORKER INSTALLER v2.6 - MOBILE EDITION     ║${NC}"
echo -e "${BLUE}║    Optimizado para Workers móviles con reconexión auto     ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [[ "$DEVICE_MODEL" == *"MacBookAir"* ]]; then
    echo -e "${GREEN}📱 MacBook Air detectado - Configurando modo Worker Móvil${NC}"
    echo -e "   (Reconexión automática cuando cambia la red)"
    echo ""
fi
...
# CRITICAL: Head Node uses Python 3.9.6 - Worker MUST match EXACTLY
# Ray requires matching Python versions for distributed clusters
echo -e "   Buscando Python 3.9.6 (versión exacta requerida)..."
...
if [ -z "$PYTHON_CMD" ]; then
    ...
    echo -e "   Descargando Python 3.9.6..."
...
if [ "$PYTHON_VERSION" != "3.9.6" ]; then
    ...
    echo -e "   El instalador debería haber instalado 3.9.6 automáticamente."
    exit 1
fi

echo -e "   ✅ Versión verificada: Python 3.9.6 (match exacto con Head Node)"
...
"$VENV_DIR/bin/pip" install "ray==2.9.0==2.51.2" "streamlit" "pandas" "ccxt" "plotly" "python-dotenv" "watchdog" "coinbase-advanced-py" "optuna" "urllib3==1.26.15" --quiet
...
<dict>
    ...
    <dict>
        ...
        <string>0</string>
        <key>RAY_IGNORE_VERSION_MISMATCH</key>
        <string>1</string>
    </dict>
</dict>
</plist>
